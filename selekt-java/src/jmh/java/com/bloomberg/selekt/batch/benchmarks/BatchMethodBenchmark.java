...
public class BatchMethodBenchmark {
    ...
    private static final int PARAMS_PER_ROW = 4; // id, name, value, data
    ...
    private String[] multiValuedSqlChunks;
    private Object[][] flattenedArgsChunks;
    ...
    private void generateTestData() {
        ...
        final Object[][] updateData = new Object[batchSize][];
        for (int i = 0; i < batchSize; i++) {
    ...
    private void generateFlattenedData() {
        final int numChunks = (batchSize + MAX_ROWS_PER_MULTI_INSERT - 1) / MAX_ROWS_PER_MULTI_INSERT;
        multiValuedSqlChunks = new String[numChunks];
        flattenedArgsChunks = new Object[numChunks][];
        int remaining = batchSize;
        int offset = 0;
        for (int chunk = 0; chunk < numChunks; chunk++) {
            int chunkSize = Math.min(remaining, MAX_ROWS_PER_MULTI_INSERT);
            multiValuedSqlChunks[chunk] = buildMultiValuedInsertSql(chunkSize);
            flattenedArgsChunks[chunk] = flattenArgs(offset, chunkSize);
            offset += chunkSize;
            remaining -= chunkSize;
        }
    }
    ...
    @Benchmark
    public void sequenceBatchMethod(final Blackhole blackhole) {
        blackhole.consume(database.batch(SQL_INSERT, sequenceBatchData));
    }

    @Benchmark
    public void sequenceBatchUpdate(final Blackhole blackhole) {
        database.batch(SQL_INSERT, sequenceBatchData);
        blackhole.consume(database.batch(
    ...
    @Benchmark
    public void multiValuedInsertManual() {
        database.transact(SQLiteTransactionMode.EXCLUSIVE, db -> {
            for (int chunk = 0; chunk < multiValuedSqlChunks.length; chunk++) {
                db.exec(multiValuedSqlChunks[chunk], flattenedArgsChunks[chunk]);
            }
            return null;
        });
    }
    ...
    private String buildMultiValuedInsertSql(final int rowCount) {
        ...
        builder.append("INSERT OR REPLACE INTO batch_test (id, name, value, data) VALUES ");
