...
internal class ExternalSQLite(
    ...
    private data class CommitHookRegistration(
        ...
        val rollbackStub: MemorySegment,
        val arena: Arena
    )
    ...
    private fun statementSegment(statement: StatementHandle): MemorySegment = when (val attachment = statement.attachment) {
        is StatementAttachment -> attachment.statement
        is MemorySegment -> attachment
    ...
    override fun storeSecret(pointer: Long, capacity: Int, source: ByteArray, length: Int) {
        ...
        val result = withSlab { slab ->
            val nativeSource = slab.allocate(length.toLong())
            try {
                MemorySegment.copy(source, 0, nativeSource, JAVA_BYTE, 0, length)
                selekt_secret_store.invoke(
                    MemorySegment.ofAddress(pointer),
                    capacity,
                    nativeSource,
                    length
    ...
    override fun bindText(
        ...
        return bindText(
            ...
            value,
            utf8TextParameters,
            attachment?.asciiText
        )
    }

    override fun bindTextAscii(statement: StatementHandle, index: Int, value: String): SQLCode {
        val attachment = statement.attachment as? StatementAttachment
        return bindTextAscii(statementSegment(statement), index, value, attachment?.asciiText)
    }
    ...
    override fun finalize(statement: StatementHandle): SQLCode = try {
        ...
        (statement.attachment as? StatementAttachment)?.release()
    }
    ...
    private fun bindBlob(
        ...
        withSlab { slab ->
            ...
            sqlite3_bind_blob.invoke(statement, index, segment, length, sqliteTransient) as Int
        }
    } else {
        val allocation = sqlite3_malloc64.invoke(length.toLong()) as MemorySegment
        if (allocation.address() == 0L) {
            SQL_NOMEM
        } else {
            val segment = allocation.reinterpret(length.toLong())
            MemorySegment.copy(blob, 0, segment, JAVA_BYTE, 0, length)
            try {
                sqlite3_bind_blob.invoke(statement, index, segment, length, sqliteFree) as Int
            } catch (failure: Throwable) {
    ...
    private fun bindText(
        ...
        value: String,
        utf8TextParameters: BooleanArray,
        reusableText: MemorySegment? = null
    ): SQLCode {
        if (utf8TextParameters[index]) {
            return bindTextUtf8(statement, index, value)
        }
        val result = bindTextAscii(statement, index, value, reusableText)
    ...
    private fun bindTextAscii(
        statement: MemorySegment,
        index: Int,
        value: String,
        reusableText: MemorySegment? = null
    ): SQLCode = if (value.length <= DIRECT_ASCII_BIND_MAX_LENGTH) {
        if (reusableText != null) {
            bindAsciiFrom(statement, index, value, reusableText)
        } else {
            withSlab { slab ->
                val text = slab.allocateFromAscii(value)
                if (text == null) {
                    SQL_MISMATCH
                } else {
                    sqlite3_bind_text.invoke(
    ...
    override fun commitHook(
        ...
        callbackRegistryLock.withLock {
            ...
            if (enabled && listener != null) {
                ...
                sqlite3_rollback_hook.invoke(segment, registration.rollbackStub, MemorySegment.NULL)
                activeListeners.put(db, registration)?.arena?.close()
            } else {
                sqlite3_commit_hook.invoke(segment, MemorySegment.NULL, MemorySegment.NULL)
                sqlite3_rollback_hook.invoke(segment, MemorySegment.NULL, MemorySegment.NULL)
                activeListeners.remove(db)?.arena?.close()
