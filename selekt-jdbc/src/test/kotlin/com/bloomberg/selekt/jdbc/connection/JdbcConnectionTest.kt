...
internal class JdbcConnectionTest {
    ...
    @Test
    fun prepareStatementWithAutoGeneratedKeysWarnsOnReturnGeneratedKeys() {
        connection.prepareStatement(
            "INSERT INTO test VALUES (?)",
            java.sql.Statement.RETURN_GENERATED_KEYS
        )
        assertNotNull(connection.warnings)
    }

    @Test
    fun prepareStatementWithAutoGeneratedKeysRejectsInvalidFlag() {
        assertFailsWith<SQLException> {
            connection.prepareStatement("INSERT INTO test VALUES (?)", 42)
        }
    }

    @Test
    fun prepareStatementWithAutoGeneratedKeysParticipatesInPool() {
    ...
    @Test
    fun prepareStatementWithColumnIndexesParticipatesInPool() {
        val sql = "INSERT INTO test VALUES (?)"
    ...
    @Test
    fun prepareStatementWithColumnNamesParticipatesInPool() {
        val sql = "INSERT INTO test VALUES (?)"
    ...
    fun setAutoCommitErrorHandling() {
        ...
        JdbcConnection(testSharedDatabase(database), connectionURL, properties).run {
            autoCommit = false
            assertFailsWith<SQLException> {
                autoCommit = true
            }
    ...
    fun ensureTransactionCalled() {
        ...
        JdbcConnection(testSharedDatabase(transactionDatabase), connectionURL, properties).apply {
            autoCommit = false
            ensureTransaction()
        }
    ...
    fun executeQueryOnReadOnlyConnectionDoesNotBeginTransaction() {
        ...
        JdbcConnection(testSharedDatabase(database), connectionURL, properties).use {
            it.isReadOnly = true
            it.autoCommit = false
